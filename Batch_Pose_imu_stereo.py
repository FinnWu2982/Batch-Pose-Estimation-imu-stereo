...
def vee(Xi):
    """Map an se(3) matrix back to a 6D vector."""
    phi = np.array([Xi[2, 1], Xi[0, 2], Xi[1, 0]])
    rho = Xi[0:3, 3]
    return np.hstack((rho, phi))

def compute_adjoint(T):
    """Return the 6x6 adjoint matrix of an SE(3) transform."""
...
def se3_inverse(T):
    """Return the inverse of an SE(3) transform."""
    C, r = T[0:3, 0:3], T[0:3, 3]
    T_inv = np.eye(4)
    T_inv[0:3, 0:3] = C.T
    T_inv[0:3, 3] = -C.T @ r
...
def compute_point_interaction_matrix(p_homo):
    """Return the point-to-pose interaction matrix."""
    p = p_homo.flatten()
    xyz = p[0:3]
    D = np.zeros((4, 6))
...
class BatchSolver:
    ...
    def integrate_dead_reckoning(self, k_start, k_end, T_initial):
        """Integrate IMU motion to form an initial pose trajectory."""
    ...
    def solve(self, k_start, k_end, initial_guess, prior_info, max_iter=15):
        """Solve the batch nonlinear least-squares problem with LM damping."""
        ...
        for iteration in range(max_iter):
            ...
            if chi2_prev is not None and total_chi2 > chi2_prev:
                ...
                lambda_val = max(1e-9, lambda_val * 0.1)
...
def main():
    ...
    visualize_results(times, solver.gt_pos[:, k1:k2 + 1], solver.gt_rot[:, k1:k2 + 1], T_sw50, cov_sw50, "Q5b Sliding Window 50")

    T_sw10, cov_sw10 = run_sliding_window(solver, k1, k2, 10)
    visualize_results(times, solver.gt_pos[:, k1:k2 + 1], solver.gt_rot[:, k1:k2 + 1], T_sw10, cov_sw10, "Q5c Sliding Window 10")
